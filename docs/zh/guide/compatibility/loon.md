...
description: 说明如何把 Loon 的远程配置、节点、插件、复写和脚本需求迁移到 Clash iOS。
head:
  - - link
    - rel: canonical
...
Loon 使用自己的节点、规则、策略、复写、脚本和插件语法。迁移到 Clash 时，服务商提供的远程配置和节点可以通过兼容格式继续使用，规则与扩展功能也可以通过对应功能重新建立。
...
1. 先尝试把同一个配置地址添加到 Clash。
2. 服务商按客户端提供多种格式时，切换 Profile User-Agent，选择 Clash / mihomo 输出。
