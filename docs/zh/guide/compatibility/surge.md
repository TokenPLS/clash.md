...
description: 说明如何把 Surge Profile 中的 Snell、Shadowsocks、Trojan、WireGuard 等节点与分流策略迁移到 Clash iOS。
head:
  - - link
    - rel: canonical
  ...
  - - script
    ...
    - '{"@context":"https://schema.org","@type":"FAQPage","mainEntity":[{"@type":"Question","name":"如何把 Surge Profile 迁移到 Clash？","acceptedAnswer":{"@type":"Answer","text":"把节点与分流策略整理为 mihomo YAML，或添加服务商提供且返回 Clash / mihomo YAML 的配置地址；模块、脚本和重写可使用 Clash 中对应的规则与功能重新建立。"}},{"@type":"Question","name":"Clash 支持 Surge 的 Snell 节点吗？","acceptedAnswer":{"@type":"Answer","text":"支持。可以通过 mihomo YAML、兼容配置地址或节点编辑器添加 Snell 节点。"}}]}'
---
...
Snell 通常通过 mihomo YAML、返回 mihomo YAML 的兼容配置地址或节点编辑器添加。
其他节点也可以根据服务器与协议参数在节点编辑器中重新添加。

## 怎么迁移？

1. 优先获取服务商提供且返回 mihomo / Clash YAML 的配置地址。
