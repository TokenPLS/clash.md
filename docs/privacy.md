...
Clash is a network utility that runs on your device. It is not a proxy service:
it connects to the services **you** configure, and it has no
developer-operated server behind it.
...
The proxy, DNS, subscription, provider, and test services you configure are
...
- **Profiles and configurations** are stored byte for byte as you supplied
  ...
  iOS data protection and the app container.
...
- **Logs and diagnostics** remain local. Clash can receive privacy-filtered
  Apple MetricKit diagnostics on the device, but it has no automatic uploader.
  Data leaves the device only when you explicitly use an export or share
  action, in which case you choose the destination.

## User-directed network connections

Clash makes network connections to provide features you configure or start:

- **Subscription, rule-provider, proxy-provider, proxy, and DNS services**
...
- **Diagnostics you start.** The network-quality test contacts Apple's
  measurement endpoint. STUN contacts the server you enter. Latency tests
  contact the URL configured for the selected group.
- **Configured background updates.** When enabled, Clash may check configured
  subscriptions and resources in the background for the purpose you selected.
...
Automatic subscription and resource updates run only when you configure or
enable them.

## iCloud backup

If you explicitly create an iCloud backup, the backup is stored in **your**
iCloud account under Apple's terms. We do not have access to it.
...
- Local profiles, settings, credentials, logs, and diagnostics remain on the
  device until you delete the relevant item, use **Clear All Local Data**, or
  uninstall Clash.
- **Clear All Local Data** removes local profiles, settings, and Keychain
  credentials. It does not delete iCloud backups.
- Current iCloud backups can be deleted from Clash's **Backup & Restore** page
  by swiping the backup and choosing **Delete**.
...
current version and update date will be published at this URL. Material changes
will also be reflected in the app or its release information.
...
Never post subscription URLs, credentials, server addresses, private logs, or
